...
const uint8_t PROGMEM RGBmatrixPanel::GFXsetBit[] = {0x80, 0x40, 0x20, 0x10,
                                                 0x08, 0x04, 0x02, 0x01};
const uint8_t PROGMEM RGBmatrixPanel::GFXclrBit[] = {0x7F, 0xBF, 0xDF, 0xEF,
                                                 0xF7, 0xFB, 0xFD, 0xFE};
#endif
...
uint16_t RGBmatrixPanel::getbaseAddr(int16_t x, int16_t y) const{
  uint16_t shift = 0;
	if (x>7) shift+=8;
	if (x>15) shift+=8;
	if (x>23) shift+=8;
...
void RGBmatrixPanel::drawPixel(int16_t x, int16_t y, uint16_t c)
{
	...
	// Calculate shift
  uint16_t shift = getbaseAddr(x, y);

	ptr = &matrixbuff[backindex][shift]; // Base addr
...
uint16_t RGBmatrixPanel::getRawPixel(int16_t x, int16_t y) const {
  if ((x < 0) || (y < 0) || (x >= WIDTH) || (y >= HEIGHT))
    return 0;
  uint16_t shift = getbaseAddr(x, y);
  if (matrixbuff) {
    return matrixbuff[0][shift];
  }
  return 0;
}
